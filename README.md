...
Pieces run anywhere from forty-five seconds to a quarter of an hour, the layer holding the foreground rotates continuously, and about a third of them carry no beat at all — the ones that do can put a soft kit right at the front. There is always a second room playing next door; walking through the doorway is how one piece becomes the next. And some nights the room turns into a club.
...
- **Night pieces** — about a fifth of movements come up as a night piece: minor-weighted harmony, vinyl surface noise running the whole way through, and a groove that owns the piece rather than decorating it. There are three of them, drawn per piece and never twice in a row, each at its own tempo:
  - **2-step, 128–140** — the kick lands on the one and then skips the third beat entirely, dropping somewhere in its second half instead, so the bar lurches rather than marches. A flat backbeat against it, hats filling the gaps unevenly. Timing offsets are fixed per pattern rather than jittered per hit — sequenced by eye, so the same hits land in the same wrong places every bar
  - **Chicago house, 120–127** — four on the floor, and the kick does not move: the whole style is built on a floor that never shifts so everything above it can. The open hat answers on the "and" of every beat, a layered clap lands on 2 and 4, and the sixteenths shuffle at around 60%, which is the entire difference between Chicago and a drum machine left on its factory setting. An organ stab on the offbeat, and a bassline that jumps the octave in the gaps the kick leaves
  - **Detroit techno, 130–140** — the same floor held dead straight, no swing and not one millisecond of humanising, because the machine being exactly a machine is the feeling. Relentless sixteenths, a clap that sometimes comes only on the 4 so the bar leans into it, rim hits syncopated against a grid that never gives, and a bass that stays under the floor and drives
- **The bassline is sidechained to the kick** — pulled down on every beat and recovering before the next, which is not a mixing trick bolted on afterwards: the pump *is* the genre's sense of motion, the thing that makes a loop feel like it is going somewhere. It runs off the transport's own quarter note rather than off the kit, because a four-four kick is on every quarter by definition and both are locked to the same clock
- **The club voices come and go with the groove** — the bassline and the stabs arrive with the kit, never before it, and leave the moment the groove does. On a night piece the trimmer is forbidden from culling any of them, so a floor cannot lose its bassline halfway through while the kick carries on over nothing
...
- **Half-time decoupling** — the transport reaches 140, but everything melodic is stretched by whichever power of two brings its felt pulse back near the resting tempo, so a night piece's pads hang under its drums instead of sprinting with them. It is what lets a club groove and an ambient orchestra be the same piece of music rather than two playing at once
...
- `?character=night` (or `open`) — pins the world. Night pieces are a
  minority draw, so this is how you reach one on demand.
- `?groove=house` (or `techno`, `two-step`) — pins which night it is, and
  implies `character=night`: asking for a groove is asking for the night it
  belongs to.

They compose: `?scale=long&groove=techno`.
...
src/
  audio/          Conductor, ConductorSkill, HarmonicField, ConductorFx,
                  ...
                  (voices: PulseKit, ClubBass, ClubStab, …)
...
- **A groove is a tempo as much as a pattern** — house slowed to 100 is not slow house, and a 2-step at 120 is not a 2-step, so each night groove gets a narrow band the Tempo knob nudges within rather than a range it sweeps. The phase sway inside that band is tiny and deliberate: a track that pushes two BPM into its peak and eases off at the end is doing what a DJ does, and holding a tempo dead flat for ten minutes is the surest way to make a groove feel like a loop
- **A clap is not one sound** — it is three or four hands not quite together, and then the room they are in. The bursts make it read as people, the tail underneath makes it read as a place, and a single noise burst with a long decay is a snare — which on 2 and 4 over a four-four kick is a rock beat
